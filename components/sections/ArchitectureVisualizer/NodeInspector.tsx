        ...
        <div className="w-full rounded-2xl border border-border bg-card p-5 md:p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono">
                ...
                <div className="rounded-xl border border-border/60 bg-background/50 p-3">
                    ...
                    <span className="font-semibold text-blue-400">{node.tech}</span>
                </div>
            </div>

            {/* Architectural Responsibilities / Details */}
            <div className="rounded-xl border border-border/80 bg-[var(--scar-code-bg)] p-4 font-mono text-xs text-slate-300 leading-relaxed">
            ...
            {node.scarId && (
                <div className="flex items-center justify-between rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-xs">
                    <div className="flex items-center gap-2 text-red-300 font-mono font-medium">
                        <Bug className="h-4 w-4 text-red-400 shrink-0" aria-hidden="true" />
                        <span>Linked Production Incident: <strong className="text-white">{node.scarId}</strong></span>
                    </div>
                    <button
                        type="button"
                        onClick={() => handleJumpToScar(node.scarId!)}
                        className="inline-flex items-center gap-1 text-xs font-mono font-bold text-red-400 hover:text-red-300 underline cursor-pointer min-h-[44px]"
                    >
                        View Scar Autopsy
