...
export default function HeroSection() {
    ...
    if (terminal.isClosed) {
        return (
            <section aria-label="System Offline State" className="relative z-10 flex min-h-[90vh] w-full flex-col items-center justify-center pt-24 pb-12 text-center px-4 animate-fade-in transform-gpu will-change-[opacity,transform]">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-red-500/20 dark:bg-red-600/10 blur-[100px] rounded-full pointer-events-none -z-10 transform-gpu"></div>
                ...
                <button
                    type="button"
                    onClick={() => { terminal.startBootSequence(); }}
    ...
    return (
        <section aria-label="Hero Section Terminal" className="relative z-10 flex min-h-[90vh] w-full flex-col items-center justify-center pt-24 pb-12 px-4">
            {!terminal.isFullScreen && (
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] md:w-[600px] md:h-[400px] bg-linear-to-r from-blue-600/10 to-purple-600/10 dark:from-blue-600/20 dark:to-purple-600/20 blur-[100px] rounded-full pointer-events-none -z-10 transform-gpu"></div>
            )}
            ...
            {!terminal.isFullScreen && (
                <div className="mb-6 inline-flex items-center justify-center gap-2 rounded-full border border-border/50 bg-background/50 backdrop-blur-md px-4 py-2 text-sm font-medium text-foreground shadow-sm transition-[background-color] hover:bg-muted/50 cursor-default animate-fade-in transform-gpu will-change-[opacity,transform]">
                    <span className="relative flex h-2 w-2">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75 transform-gpu"></span>
            ...
            {!terminal.isFullScreen && (
                <nav aria-label="Hero Action Links" className={`flex flex-wrap justify-center items-center gap-4 md:gap-5 relative z-20 transition-[opacity,transform] duration-1000 transform-gpu will-change-[opacity,transform] ${terminal.step >= 7 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"}`}>
                    <a
