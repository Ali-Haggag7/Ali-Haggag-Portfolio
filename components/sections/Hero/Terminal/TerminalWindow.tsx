...
`}
    ...
    useEffect(() => {
        if (!terminal.isFullScreen) return;
        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.key === "Escape") {
                terminal.setIsFullScreen(false);
            }
        };
    ...
    const handleToggleFullscreen = useCallback((e: React.MouseEvent) => {
        e.stopPropagation();
        terminal.setIsFullScreen((prev) => !prev);
    ...
    const bodyClasses = terminal.isFullScreen
        ? "p-6 md:p-10 space-y-4 text-gray-200 flex-1 min-h-0 h-full overflow-y-auto overflow-x-hidden break-words terminal-scrollbar text-base md:text-xl flex flex-col items-start w-full font-mono bg-[#090d16]"
    ...
    return (
        <article id="hero-terminal" className={wrapperClasses} onClick={handleTerminalClick}>
            ...
            <header className="relative z-10 flex items-center justify-between px-4 py-4 bg-[#0d1322] border-b border-gray-800 shrink-0 text-white">
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="cursor-pointer p-1 rounded-full hover:bg-white/10 transition-colors group outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
                    >
                        <div className="w-3.5 h-3.5 rounded-full bg-red-500 relative flex items-center justify-center hover:scale-110 transition-transform">
                    ...
                    <button
                        ...
                        className="cursor-pointer p-1 rounded-full hover:bg-white/10 transition-colors group outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
                    >
                        <div className="w-3.5 h-3.5 rounded-full bg-yellow-500 relative flex items-center justify-center hover:scale-110 transition-transform">
                    ...
                    <button
                        ...
                        aria-label={terminal.isFullScreen ? "Exit Fullscreen" : "Toggle Fullscreen"}
                        title={terminal.isFullScreen ? "Exit Fullscreen (Esc)" : "Fullscreen"}
                        className="cursor-pointer p-1 rounded-full hover:bg-white/10 transition-colors group outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
                    >
                        <div className="w-3.5 h-3.5 rounded-full bg-green-500 relative flex items-center justify-center hover:scale-110 transition-transform">
                            <span className="opacity-0 group-hover:opacity-100 text-[8px] font-bold text-black leading-none">+</span>
                        </div>
                    </button>
                </div>
                ...
                <div className="flex items-center gap-2 text-gray-300 text-xs font-mono select-none">
                    <Terminal className="w-4 h-4 text-emerald-400" /> ali-os — bash {terminal.isFullScreen && "[FULLSCREEN]"}
                </div>
