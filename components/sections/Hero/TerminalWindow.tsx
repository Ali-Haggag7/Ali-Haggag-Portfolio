...
export const TerminalWindow = memo(function TerminalWindow({ terminal }: { terminal: TerminalState }) {
    ...
    // PERF: Removed backdrop-blur, filter animations, grayscale, and overly complex shadows
    const wrapperClasses = terminal.isFullScreen
        ...
        : cn(
            "w-full max-w-3xl mx-auto rounded-xl overflow-hidden border border-border/50 bg-[#0D1117] shadow-xl mb-8 text-left transition-[transform,opacity] duration-300 transform-gpu",
            terminal.isMinimized ? "scale-90 opacity-60 pointer-events-none" : "scale-100 opacity-100"
        );

    // PERF: Removed transform-gpu from body to save VRAM
        ...
        <article className={wrapperClasses}>
            <header className="flex items-center justify-between px-4 py-3 bg-[#161B22] border-b border-gray-800">
                <div className="flex gap-1">
                    <button type="button" aria-label="Close Terminal" onClick={handleClose} className="relative flex items-center justify-center p-2 hover:bg-white/5 rounded-md group outline-none">
                    ...
                    <button type="button" aria-label="Minimize Terminal" onClick={handleMinimize} className="relative flex items-center justify-center p-2 hover:bg-white/5 rounded-md group outline-none">
                    ...
                    <button type="button" aria-label="Maximize Terminal" onClick={handleFullScreen} className="relative flex items-center justify-center p-2 hover:bg-white/5 rounded-md group outline-none">
            ...
            <div ref={terminal.terminalContainerRef} className={bodyClasses} role="log" aria-live="polite">
                {/* PERF: Removed all will-change and transform-gpu from list items. Browsers handle opacity fades natively very well. */}
                {terminal.step >= 1 && (
