...
export const ScarsTimelineView = memo(function ScarsTimelineView({
    ...
    return (
        <div className="relative max-w-4xl mx-auto py-6">
            ...
            <div
                aria-hidden="true"
                className="absolute left-6 md:left-1/2 top-28 bottom-6 w-0.5 -translate-x-1/2 bg-border/60 rounded-full"
            />
                ...
                {scars.map((scar, index) => {
                    ...
                    return (
                        ...
                        >
                            {/* Spine Dot Indicator */}
                            <div
                                className="absolute left-6 md:left-1/2 -translate-x-1/2 flex h-10 w-10 items-center justify-center rounded-full border-2 bg-background z-10 transition-transform duration-300 group-hover:scale-110"
                            ...
                            <div
                                className={cn(
                                    "ml-16 md:ml-0 w-full md:w-[45%] rounded-2xl p-5 border border-border bg-card shadow-sm transition-all duration-300 hover:shadow-md",
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between gap-2 mb-2">
                                    <span className="text-xs font-mono font-bold text-foreground bg-muted/40 px-2.5 py-0.5 rounded-full border border-border/40">
                                        {scar.project}
                                    </span>
                                    <span
                                ...
                                {/* Title */}
                                <h4 className="text-lg font-bold text-foreground font-mono tracking-tight">
                                    {scar.title}
                                </h4>
                                ...
                                {isExpanded && (
                                    <div className="mt-4 pt-4 border-t border-border/60 text-xs space-y-3 font-mono">
                                        <div>
                                            ...
                                            <p className="text-muted-foreground leading-relaxed">{scar.rootCause}</p>
                                        </div>
                                        <div>
                                            <span className="text-emerald-400 font-bold block mb-0.5">Fix / Resolution:</span>
                                            <p className="text-muted-foreground leading-relaxed">{scar.solution}</p>
                                        </div>
                                        {scar.codeSnippet && (
                                            <div className="rounded-xl border border-border/80 bg-[var(--scar-code-bg)] p-3 text-slate-300 overflow-x-auto">
