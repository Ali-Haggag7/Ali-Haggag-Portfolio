...
interface LiquidEtherWebGL {
  ...
  updatePalette?: (stops: string[]) => void;
  resize: () => void;
  start: () => void;
  pause: () => void;
  dispose: () => void;
...
export default function LiquidEther({
  ...
  // Stabilize colors array reference
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement) {
        ...
        this.listenerTarget.addEventListener("scroll", this._onScroll, { passive: true });
      ...
      dispose() {
        ...
        if (this.listenerTarget) {
          ...
          this.listenerTarget.removeEventListener("scroll", this._onScroll);
        }
        if (this.docTarget) {
      ...
      private isPointInside(clientX: number, clientY: number) {
        ...
        const maxBottom = this.getInteractiveBottom();
        return clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= maxBottom;
      }
...
`;
    ...
    class Output {
      ...
      updatePalette(stops: string[]) {
        const mat = this.output?.material as THREE.RawShaderMaterial | undefined;
        if (mat?.uniforms?.palette) {
          const oldTex = mat.uniforms.palette.value;
          const newTex = makePaletteTexture(stops);
          mat.uniforms.palette.value = newTex;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.interactiveElementId = props.interactiveElementId;
      ...
      updatePalette(stops: string[]) {
        if (this.output) {
          this.output.updatePalette(stops);
        }
      }
      init() {
        if (!Common.renderer) return;
  ...
  useEffect(() => {
    ...
    if (stableColors) webgl.updatePalette?.(stableColors);
    if (resolution !== prevRes) sim.resize();
  }, [
    mouseForce,
    cursorSize,
