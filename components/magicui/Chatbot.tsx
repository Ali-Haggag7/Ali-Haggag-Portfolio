...
export default function Chatbot() {
    ...
    return (
        <section aria-label="AI Assistant" className="fixed bottom-4 left-4 md:left-auto md:right-6 md:bottom-6 z-50 flex flex-col items-start md:items-end">
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="relative mb-4 w-[calc(100vw-2rem)] sm:w-[400px] h-[500px] max-h-[80vh] flex flex-col overflow-hidden rounded-2xl border border-border/50 bg-background/80 backdrop-blur-xl shadow-2xl origin-bottom-left md:origin-bottom-right"
                    >
                        ...
                        >
                            ...
                            {isLoading && (
                                <div className="flex gap-3 max-w-[85%] mr-auto" aria-label="AI is typing">
                        ...
                        <AnimatePresence>
                            {showScrollButton && (
                                <motion.button
                                    ...
                                    className="absolute bottom-[80px] left-1/2 -translate-x-1/2 bg-foreground/90 text-background rounded-full p-2 shadow-lg z-20 hover:bg-foreground hover:scale-110 active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                >
                        ...
                        <form
                            ...
                            className="border-t z-10 border-border/50 bg-background/80 p-3"
                        >
                            <div className="relative flex items-center">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-full border border-border bg-muted/50 px-4 py-3 pr-12 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500/50 shadow-sm transition-all"
                                />
                                <button
                                    aria-label="Send message"
                                    type="submit"
                                    disabled={isLoading || !input.trim()}
                                    className="absolute right-1.5 flex h-9 w-9 items-center justify-center rounded-full bg-blue-600 text-white transition-all hover:bg-blue-700 disabled:opacity-50 disabled:hover:bg-blue-600 active:scale-95 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-background"
                                >
            ...
            >
                <div className="absolute inset-0 -z-10 rounded-full bg-blue-500/40 blur-md transition-opacity group-hover:opacity-100 opacity-0" aria-hidden="true"></div>

                {isOpen ? (
                    <X className="h-6 w-6 transition-transform rotate-90 animate-in spin-in-12 duration-300" aria-hidden="true" />
                ) : (
                    <MessageSquare className="h-6 w-6 transition-transform group-hover:rotate-12" aria-hidden="true" />
                )}
            </button>
        </section>
    );
